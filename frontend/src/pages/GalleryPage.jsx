    ...
    async function load() {
      try {
        ...
        const allPhotos = await Promise.all(
          places.map((p) =>
            photosApi.list(p.id).then((photos) =>
              photos.map((ph) => ({ ...ph, placeName: p.name, placeId: p.id }))
            )
          )
        )
        setPhotos(allPhotos.flat().sort((a, b) =>
    ...
    <main className="pt-16 px-4 pb-8 max-w-[1200px] mx-auto">
      ...
      {loading && (
        <div className="columns-2 sm:columns-3 lg:columns-4 gap-3">
          {[...Array(12)].map((_, i) => (
            <div key={i} className="skeleton mb-3 break-inside-avoid rounded-lg"
